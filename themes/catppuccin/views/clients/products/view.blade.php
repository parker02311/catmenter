            ...
            <ul class="flex flex-wrap text-sm font-medium text-center text-gray-500 dark:text-gray-400">
                <li class="mr-2">
                    <a href="{{ route('clients.active-products.show', [$orderProduct->id]) }}"
                        class="inline-block p-4 text-blue-600 rounded-lg active dark:text-blue-500 border @if (request()->url() === route('clients.active-products.show', $orderProduct->id)) border-ctp-blue @else border-transparent @endif hover:border-ctp-blue hover:text-primary-400">
                        {{ __('Overview') }}
                    </a>
                </li>
                @foreach ($views['pages'] as $page)
                    <li class="mr-2">
                        <a href="{{ route('clients.active-products.show', [$orderProduct->id]) }}/{{ $page['url'] }}"
                            class="inline-block p-4 text-blue-600 rounded-lg active dark:text-blue-500 border @if (str_contains(request()->url(), $page['url'])) border-ctp-blue @else border-transparent @endif hover:border-ctp-blue hover:text-primary-400">
                            {{ $page['name'] }}
                        </a>
                    </li>
                @endforeach
