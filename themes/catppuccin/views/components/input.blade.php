@if ($type == 'checkbox')
    <div @isset($class) class="{{ $class }}" @endisset>
        <div class="flex items-start">
            <div class="flex items-center h-5">
                <input id="{{ $id ?? $name }}" type="checkbox"
                    ...
                    class="w-4 h-4 border border-secondary-300 rounded bg-ctp-mantle text-primary-400 focus:ring-3 focus:ring-primary-300 focus:ring-offset-2 ring-offset-secondary-50 dark:ring-offset-secondary-100"
        ...
        @error($name)
            <label for="{{ $id ?? $name }}" class="text-sm text-ctp-red">{{ $message }}</label>
        @enderror
    </div>
@elseif($type == 'color')
    <div @isset($class) class={{ $class }} @endisset>
        {{ $slot }}
        @isset($label)
            <label for="{{ $id ?? $name }}" class="text-sm text-ctp-subtext0">{!! $label !!}</label>
        @endisset
        <div class="relative">
            @isset($icon)
            ...
            <input type={{ $type }}
                ...
                {{ $attributes->except('class') }}
                @isset($required) {{ $required ? 'required' : '' }} @endisset
                class="bg-ctp-mantle text-secondary-800 font-medium rounded-md placeholder-secondary-500 outline-none w-full border focus:ring-2 focus:ring-offset-2 ring-offset-secondary-50 dark:ring-offset-secondary-100 duration-300
    ...
    <div @isset($class) class="{{ $class }}" @endisset>
        ...
        <div class="relative">
            ...
            <select {{ $attributes->except('class') }} id={{ $id ?? $name }}
                @isset($required) {{ $required ? 'required' : '' }} @endisset
                class="bg-ctp-mantle font-medium rounded-md outline-none w-full border focus:ring-2 focus:ring-offset-2 ring-offset-secondary-50 duration-300
            @error($name) border-ctp-red @else border-ctp-surface0 @enderror
...
@elseif($type == 'searchselect')
    <div @isset($class) class="{{ $class }}" @endisset>
        ...
        <div class="relative h-fit">
            ...
            <div id="{{$id ?? $name }}-list" class="mt-1 absolute hidden z-10 w-full bg-ctp-mantle text-secondary-800 font-medium rounded-md placeholder-secondary-500 outline-none border max-h-[100px] overflow-y-scroll focus:ring-2 focus:ring-offset-2 ring-offset-secondary-50 dark:ring-offset-secondary-100 duration-300
                @error($name) border-danger-300 focus:border-danger-400 focus:ring-danger-300 @else border-secondary-300 focus:border-secondary-400 focus:ring-primary-400 @enderror
                @isset($icon) pl-10 pr-4 @else px-4 @endisset">
                    {{ $slot }}
                </div>
    ...
    <div @isset($class) class="{{ $class }}" @endisset>
        ...
        <div class="relative">
            ...
            <textarea {{ $attributes->except('class') }} id={{ $id ?? $name }}
                @isset($required) {{ $required ? 'required' : '' }} @endisset
                class="py-2 bg-ctp-crust text-ctp-subtext0 font-medium rounded-md border placeholder-ctp-surface0 outline-none w-full focus:ring-2 focus:ring-offset-2 ring-offset-secondary-50 duration-300
            @error($name) border-ctp-red @else border-ctp-surface0 @enderror">{{ $value ?? $slot }}</textarea>
        </div>
        @error($name)
            <label for="{{ $id ?? $name }}" class="text-sm text-ctp-red">{{ $message }}</label>
        @enderror
    ...
    <div @isset($class) class="{{ $class }}" @endisset>
        ...
        <div class="relative">
            ...
            <input type={{ $type }} {{ $attributes->except('class') }}
                ...
                id={{ $id ?? $name }}
                @isset($required) {{ $required ? 'required' : '' }} @endisset
                class="bg-ctp-mantle text-secondary-800 font-medium rounded-md placeholder-secondary-500 outline-none w-full border focus:ring-2 focus:ring-offset-2 ring-offset-secondary-50 dark:ring-offset-secondary-100 duration-300 disabled:opacity-50
            @isset($icon) pl-10 pr-4 @else px-4 @endisset
            @error($name) border-danger-300 focus:border-danger-400 focus:ring-danger-300 @else border-secondary-300 focus:border-secondary-400 focus:ring-primary-400 @enderror">
        </div>
        @error($name)
    ...
    <div @isset($class) class="{{ $class }}" @endisset>
        ...
        <div class="relative">
            ...
            <input type={{ $type }} {{ $attributes->except('class') }}
                ...
                id={{ $id ?? $name }}
                @isset($required) {{ $required ? 'required' : '' }} @endisset
                class="py-2 bg-ctp-crust text-ctp-subtext0 font-medium rounded-md placeholder-ctp-surface0 outline-none w-full border focus:ring-2 focus:ring-offset-2 ring-offset-secondary-50 duration-300
            @isset($icon) pl-10 pr-4 @else px-4 @endisset
            @error($name) border-ctp-red @else border-ctp-surface0 @enderror">
        </div>
        @error($name)
