<nav class="bg-ctp-mantle">
    <div class="max-w-[1650px] mx-auto block md:flex items-center gap-x-10 px-5">
        <div class="flex justify-between md:w-auto w-full items-center">
            <a href="{{ route('index') }}" class="flex items-center font-semibold text-lg py-2 gap-x-2 w-max">
                <x-application-logo class="w-10" />
                {{ config('app.name', 'Paymenter') }}
            </a>
            <!-- Mobile menu button -->
            <div class="flex md:hidden">
                <button type="button" class="button button-secondary-outline !font-normal" onclick="openMobileMenu()">
        ...
        <div class="w-full justify-between gap-x-10 md:px-5 md:flex hidden" id="mobile-menu">
            <a href="{{ route('index') }}"
                class="md:px-2 my-1 flex items-center gap-x-1 button button-secondary-outline !font-normal">
                {{ __('Home') }}
            </a>
            @auth
            ...
            <button type="button" aria-expanded="true" data-dropdown-placement="bottom-start" aria-haspopup="true"
                ...
                <div class="absolute left-0 hidden w-56 mt-2 origin-top-right bg-ctp-crust rounded-md z-10"
                    ...
                    @foreach (App\Models\Category::whereNull('category_id')->orderBy('order')->get() as $category)
                        @if ($category->products()->where('hidden', false)->count() > 0 || $category->children->count() > 0)
                            <a href="{{ route('products', $category->slug) }}"
                                class="flex px-4 button button-secondary"
            ...
            <a href="{{ route('announcements.index') }}"
                class="md:px-2 my-1 flex items-center gap-x-1 button button-secondary-outline">
                {{ __('Announcements') }}
            </a>
            <div class="ml-auto flex items-center gap-x-1 justify-center md:pb-0 pb-4">
                ...
                @auth
                    ...
                    <button type="button" aria-expanded="true" aria-haspopup="true" data-dropdown-placement="bottom-end"
                        ...
                        <div class="absolute left-0 hidden w-60 mt-2 origin-top-right bg-ctp-crust rounded-md text-secondary-700 font-normal text-start z-10"
                             role="menu" aria-orientation="vertical" aria-labelledby="product" tabindex="-1" id="account">
                            <div class="px-2 py-2">

                                <a href="{{ route('clients.profile') }}" class="px-2 py-2 hover:bg-ctp-mantle flex items-center gap-x-2 rounded transition-all ease-in-out">
                                    <i class="ri-account-circle-line"></i> {{__('Profile')}}
                                </a>

                                {{-- <a href="#" class="px-2 py-2 hover:bg-ctp-mantle flex items-center gap-x-2 rounded"><i class="ri-instance-line"></i> {{ __('Services') }}</a> --}}

                                @if (Auth::user()->has('ADMINISTRATOR'))
                                    <a href="{{ route('admin.index') }}" class="px-2 py-2 hover:bg-ctp-mantle flex items-center gap-x-2 rounded transition-all ease-in-out">
                                        <i class="ri-key-2-line"></i> {{ __('Admin area') }}
                                    </a>
                                    <a href="{{ route('clients.api.index') }}" class="px-2 py-2 hover:bg-ctp-mantle flex items-center gap-x-2 rounded transition-all ease-in-out">
                                        <i class="ri-code-s-slash-line"></i> {{ __('API') }}
                                    </a>
                                @endif

                                <hr class="mx-2 my-1 border-ctp-subtext0" />

                                <a type="button" href="{{ route('logout') }}" onclick="event.preventDefault();document.getElementById('logout-form').submit();"
                                   class="px-2 py-2 hover:bg-ctp-mantle flex items-center gap-x-2 rounded transition-all ease-in-out"
                                >
