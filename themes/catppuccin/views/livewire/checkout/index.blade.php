                    ...
                    <table class="w-full">
                        ...
                        <tbody>
                            @foreach ($this->products as $key => $product)
                                <tr
                                    class="{{ $loop->last ? '' : 'border-b-2 border-ctp-surface0' }}">
                                    <td class="pl-6 py-3">
                                        <div class="flex">
                                        ...
                                        @error('product.' . $product->id)
                                            <span class="text-ctp-red text-sm">{{ $message }}</span>
                                        @enderror
                                    </td>
                                    <td class="py-3">
                                        @if ($product->allow_quantity == 1 || $product->allow_quantity == 2)
                                            <input type="number" name="quantity"
                                                ...
                                                class="w-16 border border-ctp-surface0 bg-ctp-crust rounded-md px-2 py-1 text-sm text-center"
                                                min="1" max="100" />
                                        @else
                                            {{ $product->quantity }}
                                        @endif
                                    ...
                                    <td class="py-3">
                                        @if ($product->discount)
                                            <span class="text-ctp-red line-through"><x-money :amount="$product->price" /></span>
                                    ...
                                    <td class="py-3 pr-6" wire:click="removeProduct({{ $key }})">
                                        <button class="button button-danger-outline">
                                            <i class="ri-delete-bin-2-line text-ctp-red"></i>
                                        </button>
                                    </td>
                                </tr>
                            @endforeach
                ...
                <div class="content-box">
                    ...
                    @else
                        <div class="flex items-center justify-between">
                            <div>
                                {{ __('Coupon:') }}
                                <span class="text-ctp-subtext0 font-semibold">{{ $coupon->code }}</span>
                            </div>
                    ...
                    @foreach ($this->products as $product)
                        @if ($product->price > 0)
                            ...
                            <div class="flex flex-row items-center justify-between -mt-1 text-ctp-subtext0 text-sm">
                        ...
                        @if ($product->setup_fee > 0)
                            <div class="flex flex-row items-center justify-between text-ctp-subtext0 text-sm">
                                <div class="flex flex-row items-center">
                                    {{ __('Setup fee') }}
                                </div>
                                <div class="flex flex-col">
                    ...
                    <form wire:submit.prevent="pay">
                        <div class="flex flex-col">
                            <label for="payment_method"
                                class="text-sm text-ctp-subtext0">{{ __('Payment method') }}</label>
                            <select id="payment_method" name="payment_method" autocomplete="payment_method"
                                wire:model.live="payment_method"
                                class="py-2 bg-ctp-crust font-medium rounded-md outline-none w-full border focus:ring-2 focus:ring-offset-2 ring-offset-secondary-50 duration-300 border-ctp-surface0 focus:ring-ctp-blue">
                        ...
                        @if (config('settings::tos'))
                            <div class="items-center p-1">
                                @php
                                $tos = "I agree to the <a href='" . route('tos') . "' class='text-ctp-blue'>terms of service</a>"; @endphp <x-input id="tos" type="checkbox" name="tos" required
