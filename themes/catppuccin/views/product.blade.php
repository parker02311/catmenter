                ...
                <div class="lg:col-span-3 col-span-12">
                    <div class="content-box">
                        <div class="flex gap-x-2 items-center">
                            <div
                                class="bg-ctp-blue w-8 h-8 flex items-center justify-center rounded-md text-gray-50 text-xl">
                                ...
                                <div class="flex flex-col gap-1 group duration-150">
                                    @if ($categoryItem->products()->where('hidden', false)->count() > 0 || $categoryItem->children->count() > 0)
                                        <a href="{{ route('products', $categoryItem->slug) }}"
                                            class="text-ctp-subtext0 @if ($category->name == $categoryItem->name || $hasActiveChild) text-ctp-text pl-3 !border-ctp-blue @endif border-l-2 border-transparent duration-300 hover:text-ctp-text hover:pl-3 hover:border-ctp-blue focus:text-ctp-text focus:pl-3 focus:border-ctp-blue">
                                            {{ $categoryItem->name }}
                                        </a>
                                    @endif
                                    @if($categoryItem->children->count() > 0)
                                        <div class="flex flex-col gap-1 @if(!$hasActiveChild) hidden @endif group-hover:flex transition ease-in-out duration-300">
                                            @foreach ($categoryItem->children as $childCategory)
                                                <a href="{{ route('products', $childCategory->slug) }}"
                                                    class="pl-6 text-sm text-ctp-subtext0 @if ($category->name == $childCategory->name) text-ctp-text pl-3 !border-ctp-blue @endif border-l-2 border-transparent duration-300 hover:text-ctp-text hover:pl-8 hover:border-ctp-blue focus:text-ctp-text focus:pl-3 focus:border-ctp-blue">
                                                    {{ $childCategory->name }}
                                                </a>
                                            @endforeach
                                        </div>
